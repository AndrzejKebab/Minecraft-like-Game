...
half scale_ao_half(const half4 curve, half index, const half intensity, const half power) {
    return pow(abs(curve[index] * intensity), power);
}
...
void compute_ao_half(const half4 curve, const half4 values, const half intensity, const half power, out half4 ao) {
    ao = half4(
        scale_ao_half(curve, values[0], intensity, power),
        scale_ao_half(curve, values[1], intensity, power),
        scale_ao_half(curve, values[2], intensity, power),
        scale_ao_half(curve, values[3], intensity, power)
    );
}
