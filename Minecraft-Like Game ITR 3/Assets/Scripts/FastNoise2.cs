...
public partial class FastNoise
{
	...
	static FastNoise()
	{
		...
		for (var id = 0; id < metadataCount; id++)
		{
			...
			for (var variableIdx = 0; variableIdx < variableCount; variableIdx++)
			{
				...
				{
					var enumCount = fnGetMetadataEnumCount(id, variableIdx);
	...
	public static FastNoise FromEncodedNodeTree(string encodedNodeTree)
	{
		...
		if (nodeHandle == IntPtr.Zero) return null;
	...
	public static float GenSingle2D(IntPtr nodeHandle, float x, float y, int seed)
	{
		return fnGenSingle2D(nodeHandle, x, y, seed);
	}

	public static float GenSingle3D(IntPtr nodeHandle, float x, float y, float z, int seed)
	{
		return fnGenSingle3D(nodeHandle, x, y, z, seed);
	}

	public static float GenSingle4D(IntPtr nodeHandle, float x, float y, float z, float w, int seed)
	{
		return fnGenSingle4D(nodeHandle, x, y, z, w, seed);
	}
	...
	[DllImport(NATIVE_LIB)]
	private static extern IntPtr fnNewFromEncodedNodeTree([MarshalAs(UnmanagedType.LPStr)] string encodedNodeTree,
	                                                      uint                                    simdLevel = 0);
