...
public partial class FastNoise
{
	...
	{
		void* noiseOutPtr = noiseOut.GetUnsafePtr();

		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);
	                                                   ...
	                                                   float  frequency,  int                seed)
	{
		...
		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);
		fnGenUniformGrid3D(nodeHandle, noiseOutPtr, xStart, yStart, zStart, xSize, ySize, zSize, frequency, seed,
		                   &minMax);
		return new OutputMinMax(minMax.x, minMax.y);
	}
	                                                   ...
	                                                   float  frequency,  int seed)
	{
		...
		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);
		fnGenUniformGrid4D(nodeHandle, noiseOutPtr, xStart, yStart, zStart, wStart, xSize, ySize, zSize, wSize,
		                   frequency, seed, &minMax);
		return new OutputMinMax(minMax.x, minMax.y);
	}
	...
	{
		void* noiseOutPtr  = noiseOut.GetUnsafePtr();
		void* xPosArrayPtr = xPosArray.GetUnsafePtr();
		void* yPosArrayPtr = yPosArray.GetUnsafePtr();

		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);
		fnGenPositionArray2D(nodeHandle, noiseOutPtr, xPosArray.Length, xPosArrayPtr, yPosArrayPtr, xOffset, yOffset,
		                     seed, &minMax);
		return new OutputMinMax(minMax.x, minMax.y);
	}
	                                                     ...
	                                                     int                seed)
	{
		...
		void* xPosArrayPtr = xPosArray.GetUnsafePtr();
		void* yPosArrayPtr = yPosArray.GetUnsafePtr();
		void* zPosArrayPtr = zPosArray.GetUnsafePtr();

		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);

		fnGenPositionArray3D(nodeHandle, noiseOutPtr, xPosArray.Length, xPosArrayPtr, yPosArrayPtr, zPosArrayPtr,
		                     xOffset, yOffset, zOffset, seed, &minMax);
		return new OutputMinMax(minMax.x, minMax.y);
	}

	public static unsafe OutputMinMax GenPositionArray4D(IntPtr nodeHandle, NativeArray<float> noiseOut,
	                                                     NativeArray<float> xPosArray, NativeArray<float> yPosArray,
	                                                     NativeArray<float> zPosArray, NativeArray<float> wPosArray,
	                                                     ...
	                                                     int seed)
	{
		...
		void* yPosArrayPtr = yPosArray.GetUnsafePtr();
		void* zPosArrayPtr = zPosArray.GetUnsafePtr();
		void* wPosArrayPtr = wPosArray.GetUnsafePtr();

		var minMax = new float2(float.PositiveInfinity, float.NegativeInfinity);

		fnGenPositionArray4D(nodeHandle, noiseOutPtr, xPosArray.Length, xPosArrayPtr, yPosArrayPtr, zPosArrayPtr,
		                     wPosArrayPtr, xOffset, yOffset, zOffset, wOffset, seed, &minMax);
		return new OutputMinMax(minMax.x, minMax.y);
	}

	[DllImport(NATIVE_LIB)]
	private static extern unsafe void fnGenUniformGrid2D(IntPtr nodeHandle, void* noiseOut,
	                                                     int    xStart,     int   yStart,
	                                                     int    xSize,      int   ySize,
	                                                     float  frequency,  int   seed, void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe uint fnGenUniformGrid3D(IntPtr nodeHandle, void* noiseOut,
	                                                     int    xStart,     int   yStart, int   zStart,
	                                                     int    xSize,      int   ySize,  int   zSize,
	                                                     float  frequency,  int   seed,   void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe uint fnGenUniformGrid4D(IntPtr nodeHandle, void* noiseOut,
	                                                     int    xStart,     int   yStart, int   zStart, int wStart,
	                                                     int    xSize,      int   ySize,  int   zSize,  int wSize,
	                                                     float  frequency,  int   seed,   void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe void fnGenTileable2D(IntPtr node,      void* noiseOut,
	                                                  int    xSize,     int   ySize,
	                                                  float  frequency, int   seed, void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe void fnGenPositionArray2D(IntPtr node,      void* noiseOut, int count,
	                                                       void*  xPosArray, void* yPosArray,
	                                                       float  xOffset,   float yOffset,
	                                                       int    seed,      void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe void fnGenPositionArray3D(IntPtr node,      void* noiseOut,  int   count,
	                                                       void*  xPosArray, void* yPosArray, void* zPosArray,
	                                                       float  xOffset,   float yOffset,   float zOffset,
	                                                       int    seed,      void* outputMinMax);

	[DllImport(NATIVE_LIB)]
	private static extern unsafe void fnGenPositionArray4D(IntPtr node,      void* noiseOut,  int   count,
	                                                       void*  xPosArray, void* yPosArray, void* zPosArray,
	                                                       void*  wPosArray,
	                                                       float  xOffset, float yOffset, float zOffset, float wOffset,
	                                                       int    seed,    void* outputMinMax);
