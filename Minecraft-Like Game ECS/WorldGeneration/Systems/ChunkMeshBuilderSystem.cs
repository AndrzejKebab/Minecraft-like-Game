...
namespace _Project.WorldGeneration.Systems
{
	...
	{
		private const    int             MAX_CONCURRENT_JOBS = 2;
		...
		private static readonly NativeArray<VertexAttributeDescriptor> layout = new(3, Allocator.Persistent)
			{
				[0] =
					new
		...
		private void ProcessJobs()
		{
			...
			for (var i = activeJobs.Count - 1; i >= 0; i--)
			{
				...
				if (EntityManager.Exists(job.Entity))
				{
					...
					chunkMeshData.ChunkMesh.RecalculateTangents();
		...
		public static int3 GetFaceDirection(int index) => index switch
		                                                  {
			                                                  0 => new int3(0, 0, -1),
			                                                  1 => new int3(0, 0, 1),
			                                                  2 => new int3(0, 1, 0),
