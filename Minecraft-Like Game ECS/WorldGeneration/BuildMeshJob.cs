...
namespace _Project.WorldGeneration
{
	...
	public struct BuildMeshJob : IJob
	{
		...
		public void Execute()
		{
			...
			for (ushort z = 0; z < ChunkSize; z++)
			{
				...
				for (var i = 0; i < meshData.Triangles.Length; i++)
				{
					...
					Vertex vert0 = CreateVertex(v0 + wPos, normal, 0, 0, texBase);
					Vertex vert1 = CreateVertex(v1 + wPos, normal, 0, 1, texBase);
					Vertex vert2 = CreateVertex(v2 + wPos, normal, 1, 0, texBase);
					Vertex vert3 = CreateVertex(v3 + wPos, normal, 1, 1, texBase);
		...
		private static Vertex CreateVertex(float3 pos, float3 norm, float u, float v, float tBase)
		{
			return new Vertex
			       {
				       Position = pos, 
				       Normal   = norm, 
				       UVs      = new float3(u, v, tBase),
			       };
		}
		...
		private void SetMeshDataArray(ref Mesh.MeshDataArray meshDataArray, NativeMesh solidMesh,
		                                     NativeMesh             fluidMesh)
		{
			...
			for (var i = 0; i < fluidIndexCount; i++)
				indexData[solidIndexCount + i] = (short)(fluidMesh.Triangles[i] + solidVertexCount);
