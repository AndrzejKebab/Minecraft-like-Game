...
{[BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Fast, FloatPrecision = FloatPrecision.Low)]
	public unsafe struct GreedyMeshJob : IJobFor
	{
		...
		public void Execute(int index)
		{
			...
			// graph-based occlusion culling data: which of the 6 chunk faces have a
			// sightline to which, through this chunk's non-opaque cells (Sodium-style).
			ECB.AddComponent(index, entity, new ChunkOcclusion { Mask = ComputeVisibilityMask(ref accessor) });
		...
		// ─────────────────────────────────────────────────────────────────────────
		// OCCLUSION VISIBILITY MASK (Sodium DirectionalVisGraph, per chunk)
		// ─────────────────────────────────────────────────────────────────────────

		// bits (from*8 + to) for all from,to in 0..5 — fully see-through chunk
		private const ulong AllFacesConnected =
			(0x3Ful << 0)  | (0x3Ful << 8)  | (0x3Ful << 16) |
			(0x3Ful << 24) | (0x3Ful << 32) | (0x3Ful << 40);

		/// <summary>
		///     Flood-fill this chunk's non-opaque cells; each connected empty region that
		///     touches a set of chunk faces means a sightline can pass between any pair of
		///     those faces. The 64-bit result (bit from*8+to) drives outward-BFS occlusion
		///     culling at render time. A solid-opaque chunk returns 0 (blocks every path).
		/// </summary>
		private ulong ComputeVisibilityMask(ref ChunkAccessor accessor)
		{
			...
			NativeArray<BlockState> center = accessor.Center;
			...
			for (var start = 0; start < n; start++)
			{
				...
				while (stack.Length > 0)
				{
					...
					if (x > 0) TryPush(ref stack, visited, center, idx - 1);
					if (x < cs - 1) TryPush(ref stack, visited, center, idx + 1);
					if (y > 0) TryPush(ref stack, visited, center, idx - 32);
					if (y < cs - 1) TryPush(ref stack, visited, center, idx + 32);
					if (z > 0) TryPush(ref stack, visited, center, idx - 1024);
					if (z < cs - 1) TryPush(ref stack, visited, center, idx + 1024);
				}
		...
		private void TryPush(ref NativeList<int> stack, NativeArray<bool> visited,
		                     NativeArray<BlockState> center, int idx)
		{
			if (visited[idx]) return;
			visited[idx] = true;
			if (IsOccluder(center[idx])) return; // marked visited so we never revisit
			stack.Add(idx);
		}
		...
		private bool IsOccluder(BlockState state)
		{
			if (state.IsEmpty || state.ID == 0) return false;
			return GetMeshType(state) == 1 && !BlockPrototypes[state.ID].IsTransparent;
		}
