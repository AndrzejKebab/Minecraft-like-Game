...
{[BurstCompile(OptimizeFor = OptimizeFor.Performance, FloatMode = FloatMode.Fast, FloatPrecision = FloatPrecision.Low)]
	public unsafe struct GreedyMeshJob : IJobFor
	{
		...
		public void Execute(int index)
		{
			...
			// refresh the visibility mask on every (re)mesh — populate produced the initial
			// one, but block edits change connectivity and re-mesh through this path
			ECB.AddComponent(index, entity,
			                 new ChunkOcclusion { Mask = ChunkVisibility.ComputeMask(accessor.Center, BlockPrototypes) });
