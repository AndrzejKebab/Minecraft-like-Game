...
public struct ThirdPersonCharacterProcessor : IKinematicCharacterProcessor<ThirdPersonCharacterUpdateContext>
{
	...
	private void HandleVelocityControl(ref ThirdPersonCharacterUpdateContext context,
	                                   ref KinematicCharacterUpdateContext   baseContext)
	{
		...
		if (characterBody.IsGrounded)
		{
			...
			CharacterControlUtilities.StandardGroundMove_Interpolated(ref characterBody.RelativeVelocity,
			                                                          ...
			                                                          deltaTime, characterBody.GroundingUp,
			                                                          characterBody.GroundHit.Normal);

			// Jump
			if (characterControl.Jump)
				CharacterControlUtilities.StandardJump(ref characterBody,
				                                       characterBody.GroundingUp * characterComponent.JumpSpeed, true,
		...
		else
		{
			...
			if (math.lengthsq(airAcceleration) > 0f)
			{
				...
				CharacterControlUtilities.StandardAirMove(ref characterBody.RelativeVelocity, airAcceleration,
				                                          characterComponent.AirMaxSpeed, characterBody.GroundingUp,
				                                          deltaTime, false);
			...
			// Gravity
			CharacterControlUtilities.AccelerateVelocity(ref characterBody.RelativeVelocity, characterComponent.Gravity,
			                                             deltaTime);
		...
		float                                 hitDistance)
	{
		...
		KinematicCharacterUtilities.Default_OnMovementHit(
		                                                  ...
		                                                  characterComponent.StepAndSlopeHandling
