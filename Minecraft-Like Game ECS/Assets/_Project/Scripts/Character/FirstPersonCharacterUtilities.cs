...
public static class FirstPersonCharacterUtilities
{
		...
		float3         characterUp)
	{
		...
		if (math.dot(characterUp, targetLookDirection) < 0f) viewPitchDegrees *= -1f;
		...
		out quaternion viewLocalRotation)
	{
		...
		var viewPitchAngleDegreesBeforeClamp = viewPitchDegrees;
		...
		quaternion characterRotation)
	{
		...
		// Pitch
		var tmpViewPitchAngleDegrees = viewPitchDegrees + pitchYawDegrees.y;
