		...
		private void Execute(Entity entity, ref OrbitCamera orbitCamera, in OrbitCameraControl cameraControl)
		{
			                                                                    ...
			                                                                    out float4x4 targetWorldTransform))
			{
				...
				quaternion cameraRotation =
					OrbitCameraUtilities.CalculateCameraRotation(targetUp, orbitCamera.PlanarForward,
					                                             orbitCamera.PitchAngle);

				// Distance input
				var desiredDistanceMovementFromInput = cameraControl.ZoomDelta * orbitCamera.DistanceMovementSpeed;
				orbitCamera.TargetDistance = math.clamp(orbitCamera.TargetDistance + desiredDistanceMovementFromInput,
				                                        orbitCamera.MinDistance, orbitCamera.MaxDistance);
...
public partial struct OrbitCameraLateUpdateSystem : ISystem
{
	...
	public void OnUpdate(ref SystemState state)
	{
		var job = new OrbitCameraLateUpdateJob
		          {
			          DeltaTime                    = SystemAPI.Time.DeltaTime,
	...
	public partial struct OrbitCameraLateUpdateJob : IJobEntity
	{
			...
			in  DynamicBuffer<OrbitCameraIgnoredEntityBufferElement> ignoredEntitiesBuffer)
		{
			...
			{
				quaternion cameraRotation =
					OrbitCameraUtilities.CalculateCameraRotation(targetWorldTransform.Up, orbitCamera.PlanarForward,
					                                             orbitCamera.PitchAngle);
				...
				orbitCamera.SmoothedTargetDistance = math.lerp(orbitCamera.SmoothedTargetDistance,
				                                               orbitCamera.TargetDistance,
				                                               MathUtilities.GetSharpnessInterpolant(orbitCamera
					                                               .DistanceMovementSharpness, DeltaTime));
...
public struct CameraObstructionHitsCollector : ICollector<ColliderCastHit>
{
	...
	private readonly float3                                               m_CameraDirection;
	private readonly Entity                                               m_FollowedCharacter;
	...
	public bool AddHit(ColliderCastHit hit)
	{
		...
		if (math.dot(hit.SurfaceNormal, m_CameraDirection) < 0f ||
