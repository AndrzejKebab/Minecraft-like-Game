...
namespace _Project
{
		...
		where T : unmanaged
	{
		...
		[NativeDisableUnsafePtrRestriction] internal UnsafePriorityQueue<T>* m_Queue;
		...
		public struct Enumerator
		{
			...
			internal AtomicSafetyHandle m_Safety;
#endif
			[NativeDisableUnsafePtrRestriction] private readonly UnsafePriorityQueue<T>* m_Queue;
			private                                              int                     m_Index;
			...
			JobHandle jobHandle = new NativePriorityQueueDisposeJob<T>
			                      {
				                      Data = new NativePriorityQueueDispose<T>
				                             { m_QueueData = m_Queue, m_Safety = m_Safety }
			                      }.Schedule(inputDeps);
...
// ---------------------------------------------------------------------------
	...
	{
		[NativeDisableUnsafePtrRestriction] public UnsafePriorityQueue<T>* m_QueueData;
		...
		public void Dispose()
		{
			UnsafePriorityQueue<T>.Free(m_QueueData);
		}
	}

	[BurstCompile]
	[GenerateTestsForBurstCompatibility(GenericTypeArguments = new[] { typeof(int) })]
	internal struct NativePriorityQueueDisposeJob<T> : IJob where T : unmanaged
	{
